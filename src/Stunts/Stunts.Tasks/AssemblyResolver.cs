...
namespace Stunts.Tasks
{
    public class AssemblyResolver : IDisposable
    {
        ...
        {
            logMessage(MessageImportance.High, $"Resolving {args.Name} for {args.RequestingAssembly?.GetName().Name}");
        ...
        private Assembly LoadAssembly(AssemblyName assemblyName)
        {
            ...
            return referencedAssemblies
                ...
                .Select(name =>
                {
                    logMessage(MessageImportance.High, $"Loading {name.Name} from {new Uri(name.CodeBase).LocalPath}");
