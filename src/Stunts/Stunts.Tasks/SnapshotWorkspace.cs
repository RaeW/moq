...
{
    internal class SnapshotWorkspace : Workspace
    {
        public static SnapshotWorkspace GetWorkspace(IBuildConfiguration configuration)
        {
            ...
            var key = typeof(SnapshotWorkspace).FullName;
            if (!(configuration.BuildEngine4.GetRegisteredTaskObject(key, lifetime) is SnapshotWorkspace workspace))
            {
                workspace = new SnapshotWorkspace(configuration);
        ...
        public SnapshotWorkspace(IBuildConfiguration configuration)
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        public async Task<Project> GetOrAddProjectAsync(string projectFullPath, Dictionary<string, (string configuration, string platform)> solutionConfig, Action<MessageImportance, string> log, CancellationToken cancellation)
        {
            ...
            project = await AddProjectAsync(projectFullPath, solutionConfig, log, cancellation);
        ...
        private async Task<Project> AddProjectAsync(string projectFullPath, Dictionary<string, (string configuration, string platform)> solutionConfig, Action<MessageImportance, string> log, CancellationToken cancellation)
        {
            await initializer;
            ...
            var project = await AddProjectFromMetadataAsync(metadata, solutionConfig, log, cancellation);

            return project;
        }

        private async Task<Project> AddProjectFromMetadataAsync(dynamic projectMetadata, Dictionary<string, (string configuration, string platform)> solutionConfig, Action<MessageImportance, string> log, CancellationToken cancellation)
        {
            var watch = Stopwatch.StartNew();
            ...
            foreach (var projectReference in projectMetadata.ProjectReferences)
            {
                ...
                {
                    referencedProject = await AddProjectFromMetadataAsync(projectReference, solutionConfig, log, cancellation);
                }
