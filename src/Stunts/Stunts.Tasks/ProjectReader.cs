...
namespace Stunts.Tasks
{
    internal class ProjectReader : IDisposable
    {
        public static ProjectReader GetProjectReader(IBuildConfiguration configuration)
        {
            ...
            {
                reader = new ProjectReader(configuration.MSBuildBinPath, configuration.ToolsPath);
        ...
        public ProjectReader(string msBuildBinPath, string toolsPath)
        {
        ...
        private void EnsureRunning()
        {
            if (process == null || process.HasExited)
            {
                ...
                var args = (Debugger.IsAttached ? "-d " : "") +
                    "-m=\"" + msBuildBinPath + "\"" +
        ...
        {
            EnsureRunning();
            await rpc.InvokeAsync(nameof(CreateWorkspaceAsync), globalProperties);
        }
